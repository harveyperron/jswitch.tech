...
$blockdate= date("Y-m-d", $dateTimestamp); 

$query="update calendar set special_day=false where block='".$block."'";
if($result = $conn-> query($query)){
	$query="update calendar set is_available=true,other=null where block='".$block."' and is_booked=false and regular=false";
	if($result = $conn-> query($query)){
		$this_date=$blockdate;
		echo "<table>";
