...
echo "var special_days=["; 
	$query="SELECT distinct date(block) as partly_unavailable_date from calendar where special_day=true and regular=false";
	if($result = $conn-> query($query)){
		$n_row=mysqli_num_rows($result);
		$i_row=0;
	    while ($row = $result->fetch_assoc()){
			echo "'".$row['partly_unavailable_date']."'"; 
			if($i_row<($n_row-1)){
				echo ",";
			}
			$i_row++;
...
setInterval(function(){
	...
	for(i=0;i<tds.length;i++){
		...
		if(booked_dates.indexOf(date_last_month)>-1){
		...
		}else if(special_days.indexOf(date_last_month)>-1){
			tds[i].style.backgroundColor='gold';
		}
	}
	...
	for(i=0;i<tds.length;i++){
		...
		if(booked_dates.indexOf(date_this_month)>-1){
		...
		}else if(special_days.indexOf(date_this_month)>-1){
			tds[i].style.backgroundColor='gold';
		}
	}
	...
	for(i=0;i<tds.length;i++){
		...
		if(booked_dates.indexOf(date_next_month)>-1){
		...
		}else if(special_days.indexOf(date_next_month)>-1){
			tds[i].style.backgroundColor='gold';
		}
	}
	$('td.day.active').css('background-color','#337ab7');
