...
document.querySelectorAll('.bootstrap-datetimepicker-widget')[0].style.width='300px';
...
setInterval(function(){
	...
	for(i=0;i<tds.length;i++){
		...
		if(partially_booked_dates.indexOf(date_last_month)>-1){
	...
	for(i=0;i<tds.length;i++){
		...
		if(partially_booked_dates.indexOf(date_this_month)>-1){
			if(new Date() <= new Date(date_this_month))tds[i].style.backgroundColor='lightgreen';
			else if(new Date() > new Date(date_this_month)){tds[i].style.backgroundColor='pink';}
		}
	}
	...
	for(i=0;i<tds.length;i++){
		...
		if(partially_booked_dates.indexOf(date_next_month)>-1){
