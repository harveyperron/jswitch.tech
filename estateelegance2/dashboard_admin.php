...
foreach($upcoming_orders_ids as $order_id){
	if ($result = $conn-> query("SELECT * from orders where order_id=".$order_id." and date(appointment_datetime) > date(now()) and order_status='ordered'")){
    	while($row = $result->fetch_assoc()){
			...
			$anytime_on_date=$row["anytime_on_date"];
			$on_site=$row["on_site"];
			$additional_info=$row["additional_info"];
			$appointment_datetime=$row["appointment_datetime"];
			$appointment_date = substr($appointment_datetime,0,-9);
	        echo "<div rel=".$order_id." name='".$appointment_date."' class=upcoming>";
			date_default_timezone_set("America/New_York"); 
			$from = strtotime($appointment_datetime);
			$today = time();
			$difference = $from - $today;
			$in_days=ceil($difference / 86400);  // (60 * 60 * 24)

			echo "<img class=delete_order name=$order_id style='float:right;' src='https://spng.pngfind.com/pngs/s/57-578431_trash-can-clipart-general-waste-trash-bin-icon.png'></img>";
			...
			echo "<div>Appointment time: ".$appointment_datetime."</div>";
			echo "<div>Anytime on date: ".$anytime_on_date."</div>";
			echo "<div>On site: ".$on_site."</div>";
			echo "<div>Additional: ".$additional_info."</div>";
			echo "<b> $".$service['order_price']."</b>"." paid with ".$row['payment_method'];
			echo "<div >Client: ".$client_firstname." ".$client_lastname. "<br><a href='mailto:".$client_email."'>".$client_email."</a><br><a href='tel:+".$client_phone."'>".$client_phone."</a></div>";
					echo "</div>";
			echo "</div>";
		}
	}
...
foreach($waiting_orders_ids as $order_id){
	if ($result = $conn-> query("SELECT * from orders where order_id=".$order_id." and date(appointment_datetime)<=date(now()) and order_status='ordered'")){
		while($row = $result->fetch_assoc()){
			...
			$appointment_datetime=$row["appointment_datetime"];
			$appointment_date = substr($appointment_datetime,0,-9);
	        echo "<div rel=".$order_id." name='".$appointment_date."' class=awaiting>";
			echo "<img class=delete_order name=".$order_id." style='float:right' src='https://spng.pngfind.com/pngs/s/57-578431_trash-can-clipart-general-waste-trash-bin-icon.png'></img>";
			...
			foreach($requested_services as $service){
				if($service['service']){
					...
					echo "</div><div class=fha><input class=google_drive_link_input name=\"".$order_id."\" placeholder=\"link to client's products\" type=text></input><div class=send_button>Deliver</div></div>";
					echo "<br>";
					echo "</div>";
...
echo "<label>
  <input placeholder='Block name' style='width:120px' class=question name=other type=text value=\"\">
</label>";
